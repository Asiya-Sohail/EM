  ...
  if($email == '') { $errors[] = 'email is missing'; }
    ...
    if ( empty($errors) ) {
       ...
       $result = mysqli_query($connection,$query);
			if ($result) {
				$message = "The profile was successfully updated.".$track;
			} else {
...
if (array_key_exists('chcore', $_POST)) {
    $errors = array();
    $errors="";
	#echo "Outside track".$_SESSION['track'];
	if($_SESSION['track'] == 6)
	{
	#echo "Inside track 6";
	#echo $_POST['semester_EM531'];
  ...
  <div align= 'center'> <br/>
    <table border= "1" cellspacing="0">
         <tr><th colspan="5"><b><?if($row['track']==1){echo "Design and Manufacturing Track F2007";} if( $row['track']==2){echo "Lean Enterprise Systems Track F2007";} if( $row['track']==3){echo "Project/Program Management Track 2007";} if( $row['track']==4){echo "Quality Certificate Track F2007";}if( $row['track']==5){echo "R & D (Development Project) Track F2007";}if( $row['track']==6){echo "New Curriculum Fall 2018";}?> (Hours Required 18 hrs)</b></th></tr>
          ...
          <?php $pos_id=$_SESSION['pos_id'];?>
          <?php if($row['track']==1){ echo 'track1'; echo getcname('EM511',$pos_id); echo getcname('EM558',$pos_id); echo getcname('EM620',$pos_id); echo getcname('EM636',$pos_id); echo getcname('EM649',$pos_id); echo getcname('EM670',$pos_id);}?>
          <?php if($row['track']==2){ echo 'track2';echo getcname('EM556',$pos_id); echo getcname('EM558',$pos_id); echo getcname('EM620',$pos_id); echo getcname('EM649',$pos_id); echo getcname('EM659',$pos_id); echo getcname('EM669',$pos_id);}?>
          <?php if($row['track']==3){ echo 'track3';echo getcname('EM519',$pos_id); echo getcname('EM547',$pos_id); echo getcname('EM556',$pos_id); echo getcname('EM609',$pos_id); echo getcname('EM620',$pos_id); echo getcname('EM649',$pos_id);}?>
          <?php if($row['track']==5){ echo 'track5'; echo getcname('EM547',$pos_id); echo getcname('EM620',$pos_id); echo getcname('QUAL647',$pos_id); echo getcname('EM691',$pos_id); echo getcname('EM690',$pos_id); echo getcname('QUAL551',$pos_id); echo getcname('QUAL557',$pos_id);echo getcname('QUAL651',$pos_id);}?>
          ...
          <?php if($row['track']==4){ //this is quality track that is looklike elective classes
                ...
                $str .= "<tr colspan='6'><th colspan='6'><b>Restricted Quality Track Courses(Hours Required 18 hrs)</b></th></tr>";
                 $eid=$_SESSION['pos_id'];
				 echo "SELECT * FROM elective WHERE elective.EID='".$eid."' AND elective.qualitytrack='1' ";
                ...
                /*else{ echo "<tr colspan='6'><td colspan='6' align='right'><input type='submit' name='submit' value='Submit' /></td></tr>"; }*/
               ?>
		<?php if($row['track']==6){//2018 track elective classes
                echo 'track6';
				$str="";
  ...
  <div align= 'center'> <br/>
    <table border= "1" cellspacing="0">
         ...
         <tr colspan="6"><th colspan="6"><b> Elective Courses</pre></b></th></tr>
          <?php
          $eid=$_SESSION['pos_id'];
		  echo "SELECT * FROM elective WHERE elective.EID='".$eid."' AND elective.qualitytrack=0";
...
<a href="" onclick="printSelection(document.getElementById('test'));return false"><img src="style/images/print.jpg" /></a> <br/>
    ...
    <div id="test">
     ...
     <div align= 'center'>
      <table border= "1" cellspacing="0">
         ...
         <?php $pos_id=$_SESSION['pos_id']; ?>
		<?php if ($row['track']==6) { ?>
		<?php echo reportpos('EM505',$pos_id); echo reportpos('EM509',$pos_id); echo reportpos('EM520',$pos_id); echo reportpos('EM540',$pos_id); echo reportpos('EM531',$pos_id); echo reportpos('EM620',$pos_id); } 
    ...
    <!--*****Track Section Summery******-->
          ...
          <?php if($row['track']==4){//this is quality track that is looklike elective classes
                 ...
                 $eid=$_SESSION['pos_id'];
				 $result = mysqli_query($connection,"SELECT distinct * FROM elective WHERE elective.EID='".$eid."' AND elective.qualitytrack='1' ");
                 //$str="";
                 while($row = mysqli_fetch_assoc($result)){
                            $str .="<tr><td>".$row['name']."</td>";
                            $str .="<td>".$row['chr']."</td>";
