...
//echo $_POST['EMCID'].'-'.$_POST['courseyear'].'-'.$_POST['coursesemester'];
    ...
    $str = "SELECT * FROM course, courseteam WHERE courseteam.CID = course.CID";
	if($_REQUEST['EMCID'] <> '') 
	{
		$str .=" AND course.EMCID = ".$_REQUEST['EMCID'];
	}
	if($_REQUEST['courseyear'] <> '') 
	{
		$str .=" AND course.year=".$_REQUEST['courseyear'];
	}
	if($_REQUEST['coursesemester'] <> '') 
	{
		$str .=" AND course.semester='".$_REQUEST['coursesemester']."'";
	}
	$str .=" ORDER BY courseteam.ctname ";
    echo $str;
    ...
    if (isset($_REQUEST['student'])) {
        ...
        $q_str = mysqli_query($connection,$teamstr);
		$teamresult = mysqli_fetch_assoc($q_str);
        //print_r($teamresult); 
        ...
        while ($row = mysqli_fetch_assoc($result)) {
			$_SESSION['CTID'] = $row['CTID'];
			$_SESSION['tid'] = $row['CMTID'];
            $tbstr .= "<tr><td>";
...
//adding $totalRow10 = 0;
            $rate = array();
            while ($rowdata = mysqli_fetch_assoc($tmresult)) {
                $mean = 0;
                $meanoftotal = 0;
                $totalmean = 0;
                $num +=1;
...
//adding rate10
                ...
                if ($rowdata['rate10'] != 0) {
                    $totalRow10 = $totalRow10+$rowdata['rate10'];
                    $meanoftotal = $meanoftotal + 1;
                    $mean = $mean + $rowdata['rate10'];
                }
//adding if for rate10
                ...
                //$strm .= "<tr><td>".$num."</td><th>".$tmrow['idea']."</th><td>".ROUND(mean($rate),2)."</td><th>".median($rate)."</th><td>".ROUND(sd($rate),3)."</td><th>".max($rate)."</th><td>".min($rate)."</td></tr>";
//adding <td>" . $rowdata['rate10'] . "</td>
                $strm .= "<tr><td>" . $num . "</td><td>" . $member['firstname'] . " " . $member['lastname'] . "</td><td>" . $rowdata['rate1'] . "</td><td>" . $rowdata['rate2'] . "</td><td>" . $rowdata['rate3'] . "</td><td>" . $rowdata['rate4'] . "</td><td>" . $rowdata['rate5'] . "</td><td>" . $rowdata['rate6'] . "</td><td>" . $rowdata['rate7'] . "</td><td>" . $rowdata['rate8'] . "</td><td>" . $rowdata['rate9'] . "</td><td>" . $rowdata['rate10'] . "</td><td>" . $totalmean . "</td></tr>";
            }
//adding Q11,12                
            
            $strm .= "<tr><td colspan='2' style='text-align:center;'>Mean</td><td>" . round($totalRow1 / $num, 2) . "</td><td>" .  round($totalRow2 / $num, 2) . "</td><td>" .  round($totalRow3 / $num, 2) . "</td><td>" .  round($totalRow4 / $num, 2) . "</td><td>" .  round($totalRow5 / $num, 2) . "</td><td>" .  round($totalRow6 / $num, 2) . "</td><td>" .  round($totalRow7 / $num, 2) . "</td><td>" .  round($totalRow8 / $num, 2) . "</td><td>" .  round($totalRow9 / $num, 2) . "</td><td>" .  round($totalRow10 / $num, 2) . "</td><td colspan='3'></td></tr>";
            $strm .='<tr><td colspan="15" style="text-align:center;">Total Weighted Score : '.round($totalmeans/$num,2).'<a href="teamrating.php?teamrating=1&student=1&skilldevelopment=1&EMCID=' . $_REQUEST["EMCID"] . '&courseyear=' . $_REQUEST["courseyear"] . '&coursesemester=' . $_REQUEST["coursesemester"] . '&CTID=' . $_SESSION["CTID"] .'&evalution=1&tid=' . $_REQUEST['tid'] . ' "style="text-decoration:none"> <input type="submit" name="submit" value="Skill Development" /></a> <a href="teamrating.php?teamrating=1&student=1&overallcomment=1&EMCID=' . $_REQUEST["EMCID"] . '&courseyear=' . $_REQUEST["courseyear"] . '&coursesemester=' . $_REQUEST["coursesemester"] . '&CTID=' . $_SESSION["CTID"] .'&evalution=1&tid=' . $_REQUEST['tid'] . ' " style="text-decoration:none"> <input type="submit" name="submit" value="Overall Comment" /></a></td>';
			
			
            $strm .= "</table>";
            echo $strm;
//adding <td>" .  round($totalRow10 / $num, 2) . "</td>
        }
    }
	
	
	if (isset($_REQUEST['skilldevelopment'])) {
       $teid = $_REQUEST['tid'];
       $memStr = "SELECT * FROM cmteam em join member mem on mem.EID = em.EID WHERE em.CMTID=".$teid;
        $memresult = mysqli_fetch_assoc(mysqli_query($connection,$memStr));
        //print_r($memresult);
        ...
        if (!empty($teid)) {
            $tmresult = mysqli_query($connection,"Select * From peerevaluation WHERE CMTID='".$_REQUEST['tid']."' GROUP BY EID ORDER BY PEID DESC ");
            $strm .="<table class='sortable' border='1' cellspacing='0'><tr><u>Skill Development</u></tr>";
//adding <th title='Considering the whole team experience, provide an overall rating of this teammate'><u>Rating 10</u></th>
//adding new column names to first row
            $meanofmeans = 0;
            $totalmeans = 0;
...
//adding $totalRow10 = 0;
            ...
            while ($rowdata = mysqli_fetch_assoc($tmresult)) {
                ...
                $strm .= "<tr><td>" . $rowdata['skilldev'] . "</td></tr>";
            }
//adding Q11,12                
...
//adding <td>" .  round($totalRow10 / $num, 2) . "</td>
	...
	if (isset($_REQUEST['overallcomment'])) {
       $teid = $_REQUEST['tid'];
       $memStr = "SELECT * FROM cmteam em join member mem on mem.EID = em.EID WHERE em.CMTID=".$teid;
        $memresult = mysqli_fetch_assoc(mysqli_query($connection,$memStr));
        //print_r($memresult);
        ...
        if (!empty($teid)) {
            $tmresult = mysqli_query($connection,"Select * From peerevaluation WHERE CMTID='".$_REQUEST['tid']."' GROUP BY EID ORDER BY PEID DESC ");
            $strm .="<table class='sortable' border='1' cellspacing='0'><tr><u>Overall Comment</u></tr>";
//adding <th title='Considering the whole team experience, provide an overall rating of this teammate'><u>Rating 10</u></th>
//adding new column names to first row
            $meanofmeans = 0;
            $totalmeans = 0;
...
//adding $totalRow10 = 0;
            ...
            while ($rowdata = mysqli_fetch_assoc($tmresult)) {
                ...
                $strm .= "<tr><td>" . $rowdata['comment'] . "</td></tr>";
            }
//adding Q11,12                
...
// Item is found. Set its property and exith
